...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/basir96/izin/ipuk/ip | grep $MYIP | awk '{print $2}')
...
LLatest=`date`
Get_Data () {
git clone https://github.com/citralinggau/backup.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    ...
    git config --global user.name "citralinggau" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    git commit -m backup &> /dev/null
    git branch -M main &> /dev/null
    git remote add origin https://github.com/citralinggau/backup
    git push -f https://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@github.com/citralinggau/backup.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/citralinggau/backup/main/$NameUser/$NameUser.zip"
sleep 1
